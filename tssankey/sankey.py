...
def cwcurve_poly (x_left, x_right, base_left, base_right, height, xy_ratio=1, steps=50, **kwargs):
    ...
    ang = np.arctan2(ydiff, xdiff) + np.pi / 2 # rotate 90 degrees from direction of line
    #print(np.degrees(ang))
    for i, offset in enumerate([height / 2, - height / 2]):
        #print(i, offset)
        xoff = np.array([*(np.cos(ang) * offset), 0]) * xy_ratio # handle last offset due to fencepost problem
